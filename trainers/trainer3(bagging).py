...
class Trainer(AbstractTrainer):
    ...
    def calculate_metrics(self, batch_outlist, average_meter_set):
        batch_out = batch_outlist[0]
        outs = [batch_outlist[i]['outs'] for i in range(5)]
        outs = torch.stack(outs, 0).mean(0)
