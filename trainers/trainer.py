...
class Trainer(AbstractTrainer):
    ...
    def calculate_loss(self, batch_out, **kwargs):
        ...
        if self.learning_pattern != 'separate' and self.base_models_num > 1:
            kd_loss = self.kd_loss_peers(logits, one_hot_labels, masked_lm_weights, current_epoch, top_order)
            loss += kd_loss
        return loss


    def kd_loss_peers(self, logits, one_hot_labels, masked_lm_weights, current_epoch, top_order):
        ...
        for peer_id in range(num_peers):
            if self.learning_pattern in ['mutual_target', 'mutual_all','mutual_target_alpha', 'mutual_target_alpha_nontarget']:
                # target_loss
                arr1 = logits[0:peer_id]
                arr2 = logits[peer_id + 1:]
                tea_target_logit = torch.cat((arr1, arr2), dim=0).mean(dim=0)
                ...
                if self.learning_pattern in ['mutual_target_alpha', 'mutual_target_alpha_nontarget']:
                ...
                target_numerator = torch.sum(target_numerator, -1)
            ...
            if self.learning_pattern in ['mutual_nontarget', 'mutual_all', 'mutual_target_alpha_nontarget']:
                ...
                tea_nckd_logit = torch.cat((arr1, arr2), dim=0).mean(dim=0)
                ...
                nckd_numerator = masked_lm_weights * (-torch.sum(nckd_numerator, -1))
                nckd_numerator = torch.sum(nckd_numerator, -1)
                denominator = torch.sum(masked_lm_weights) + 1e-5
                non_target_loss += nckd_numerator / denominator
