...
class Model(nn.Module):
    def __init__(self, args):
        ...
        encoder_layers = nn.TransformerEncoderLayer(d_model=hidden_units, nhead=self.bert_num_heads,
                                                 dim_feedforward=bert_intermediate_size, dropout=bert_dropout, activation="gelu", batch_first=True, norm_first=True)
        BERT = nn.TransformerEncoder(encoder_layers, num_layers=bert_num_layers)
        ...
        self._save_case_study_embed_table(self.embedding.embedding_table())
    ...
    def forward(self, batch_input, inputs_other=None):
        ...
        self._save_case_study_embed_seq(model_outs, batch_input['info'], batch_input['masked_lm_weights'])
