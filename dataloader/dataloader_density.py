...
class Dataloader(object):
    ...
    def load_dataset(self):
        ...
        # np.random.seed(args.random_shuffle_seed)
        # train_dataset.bagging()
    ...
    def _data_partition(self):
        ...
        for user in User:
            ...
            # density
            if (self.args.density is not None) and (nfeedback > 5):
                seq_num = int(nfeedback * self.args.density)
                index = np.sort(np.random.choice(list(range(nfeedback)), seq_num, replace=False))
                User[user] = list(User[user][i] for i in index)
                nfeedback = len(User[user])
