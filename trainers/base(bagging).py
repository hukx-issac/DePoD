...
class AbstractTrainer(metaclass=ABCMeta):
    def __init__(self, args, modellist, train_loader, test_loader, export_root):
        self.args = args
        self.device = torch.device("%s"%args.device if torch.cuda.is_available() else "cpu")
        self.modellist = [model.to(self.device) for model in modellist]
    ...
    def test(self, modellist):
        self.modellist = [model.to(self.device) for model in modellist]
        print('\nTest model with test set!')
        experiment_dir, experiment_description, dataset_code = self.args.experiment_dir, self.args.experiment_description, self.args.dataset_code
        for i in range(5):
            export_root = os.path.join(experiment_dir, dataset_code,
                                       (experiment_description + str(i+1) + "_" + self.args.resume_experiment_dir))
            model = modellist[i]
            test_model_path = os.path.join(export_root, 'models', 'checkpoint-recent.pth')
        ...
        with torch.no_grad():
            ...
            for batch_idx, batch in enumerate(tqdm_dataloader):
                ...
                for model in self.modellist:
                    batch_outlist.append(model(batch_input))
                average_meter_set = self.calculate_metrics(batch_outlist, average_meter_set)
